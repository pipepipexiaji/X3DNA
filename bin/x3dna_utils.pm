...
$url    = "Check URL: http://3dna.rutgers.edu/ for the latest";
...
sub render_image {
    ...
    my ( $r3dfile, $imgfile, $pymol ) = @_;
    my $X3DNA_BIN = x3dna_bin();

    if ($pymol) {    # ray-traced with PyMOL
        $pymol = 96 unless ($pymol); # resolution
        system("$X3DNA_BIN/x3dna_r3d2png -p -r=$pymol $r3dfile $imgfile");
    } else {         # rendered with Raster3D
        system("$X3DNA_BIN/x3dna_r3d2png $r3dfile $imgfile");
    }
}
